...
export default function AdminDashboard() {
  ...
  const [pageviewsData, setPageviewsData] = useState<AnalyticsData | null>(
    null
  );
  const [visitorsData, setVisitorsData] = useState<AnalyticsData | null>(null);
  const [referrersData, setReferrersData] = useState<AnalyticsData | null>(
    null
  );
  const [browsersData, setBrowsersData] = useState<AnalyticsData | null>(null);
    ...
    try {
      ...
      const responses = await Promise.all(
        endpoints.map((endpoint) =>
          fetch(
            `/api/analytics?endpoint=${endpoint}&timeframe=${analyticsTimeframe}`
          )
        )
      );

      const [pageviews, visitors, referrers, browsers, devices, timeseries] =
        await Promise.all(responses.map((res) => res.json()));

      setPageviewsData(pageviews);
      setVisitorsData(visitors);
      setReferrersData(referrers);
      setBrowsersData(browsers);
  ...
  // Fetch analytics when tab changes or timeframe changes
  useEffect(() => {
    if (activeTab === "analytics" && allowed) {
      fetchAnalytics();
    }
  }, [activeTab, allowed, fetchAnalytics]);
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                ...
                <div className="flex items-center gap-3">
                  <select
                    ...
                    className="rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm text-zinc-700 focus:outline-none focus:ring-2 focus:ring-red-500"
                  >
                ...
                <>
                  ...
                  {timeseriesData?.data && timeseriesData.data.length > 0 && (
                    <div className="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm">
                      ...
                      <ChartContainer
                        config={{
                          pageViews: { label: "Page Views", color: "#dc2626" },
                  ...
                  <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
                    ...
                    <div className="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm">
                      <div className="flex flex-col items-center">
                        ...
                        <p className="text-sm text-zinc-500">Unique Visitors</p>
                        <p className="text-3xl font-bold text-zinc-900">
                          {pageviewsData?.data
                            ?.reduce((sum, item) => sum + item.devices, 0)
                            ?.toLocaleString() || "0"}
                        </p>
                      </div>
                    </div>
                              ...
                              >
                                ...
                                <div className="flex items-center gap-4 text-sm">
                                  <span className="text-zinc-500">
                                    {page.devices.toLocaleString()} visitors
                                  </span>
                                  <span className="font-medium text-zinc-900">
                                    {page.total.toLocaleString()} views
                                  </span>
                                </div>
                              </div>
                            ))}
                          ...
                          {visitorsData.data
                            ...
                            .map((country, index) => (
                              ...
                              >
                                ...
                                <div className="flex items-center gap-4 text-sm">
                                  <span className="text-zinc-500">
                                    {country.devices.toLocaleString()} visitors
                                  </span>
                                  <span className="font-medium text-zinc-900">
                                    {country.total.toLocaleString()} views
                                  </span>
                                </div>
                              </div>
                            ))}
                  ...
                  <div className="grid gap-6 lg:grid-cols-2">
                    ...
                    <div className="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm">
                      ...
                      {browsersData?.data && browsersData.data.length > 0 ? (
                        <ChartContainer
                          config={chartConfig}
                  ...
                  <div className="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm">
                    ...
                    {referrersData?.data && referrersData.data.length > 0 ? (
                      <div className="space-y-3">
                        {referrersData.data
                          .slice(0, 10)
                          .map((referrer, index) => (
                            ...
                            >
                              ...
                              <div className="flex items-center gap-4 text-sm">
                                <span className="text-zinc-500">
                                  {referrer.devices.toLocaleString()} visitors
                                </span>
                                <span className="font-medium text-zinc-900">
                                  {referrer.total.toLocaleString()} visits
                                </span>
                              </div>
                            </div>
                          ))}
